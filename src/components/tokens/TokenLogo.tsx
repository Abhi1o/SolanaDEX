...
export function TokenLogo({ token, size = 'md', className = '' }: TokenLogoProps) {
  ...
  const generateFallbackIcon = (symbol: string) => {
    ...
    return (
      <div className={`${sizeClass} ${bgColor} rounded-full flex items-center justify-center text-white font-bold ${className}`}>
  ...
  return (
    <div className={`${sizeClass} ${className} relative`}>
      {isLoading && (
        <div className={`${sizeClass} bg-gray-200 rounded-full animate-pulse`} />
      )}
      <img
        src={token.logoURI}
        alt={`${token.symbol} logo`}
        className={`${sizeClass} rounded-full object-cover ${isLoading ? 'opacity-0' : 'opacity-100'} transition-opacity`}
